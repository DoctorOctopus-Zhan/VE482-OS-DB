...
int main(int argc, char *argv[]) {
  ...
  if (parsedArgs.threads < 0) {
    ...
    // long threadNum = sysconf(_SC_NPROCESSORS_CONF);
    threadNum = (long)std::thread::hardware_concurrency();
  ...
  size_t counter = 0;
  ThreadPool *poolPtr = new ThreadPool(threadNum);
  //  ThreadPool pool(threadNum);
  ThreadPool::initInstance(poolPtr);
