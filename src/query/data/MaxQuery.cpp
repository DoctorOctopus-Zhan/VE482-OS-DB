        ...
        for (auto it = table.begin(); it != table.end(); ++it) {
          if (this->evalCondition(*it)) {
            for (Table::FieldIndex i = 0; i < this->operands.size(); ++i) {
              fieldValue[i] = max(fieldValue[i], (*it)[fieldId[i]]);
            }
          }
        }
      }
      if (fieldValue[0] == Table::ValueTypeMin) {
        // table.readUnlock();
...
std::vector<Table::ValueType> MaxQuery::partialExecute(Table::Iterator begin,
                                                       Table::Iterator end,
                                                       Table &table) {
  using namespace std;
  // vector storing the field min, initial Table::ValueTypeMin
...
QueryResult::Ptr MaxQuery::threadExecute(int subQuerySize) {
  using namespace std;
  Database &db = Database::getInstance();
  auto &table = db[this->targetTable];
  int counter = 0;
  ...
  /* take max of all */
  fieldType totalResult(this->operands.size(), Table::ValueTypeMin);
  for (auto &future : futures) {

    auto result = future.get();
    for (unsigned long i = 0; i < totalResult.size(); ++i) {
      totalResult[i] = max(totalResult[i], result[i]);
    }
  }

  if (totalResult[0] == Table::ValueTypeMin) {
    return make_unique<NullQueryResult>();
  }
