      ...
      if (result.second) {
        for (auto it = table.begin(); it != table.end(); ++it) {
          if (this->evalCondition(*it)) {
            Table::ValueType fieldResult = (*it)[fieldId[0]];
            ++counter;
            for (size_t i = 1; i < fieldNum - 1; ++i) {
...
Table::SizeType SubQuery::partialExecute(Table::Iterator begin,
...
QueryResult::Ptr SubQuery::threadExecute(int subQuerySize) {
  using namespace std;
...
//          fieldResult -= (*it)[fieldId[i]];
//        }
//        table.add_subByIndex(count, fieldId[fieldNum - 1], fieldResult);
//      }
//      ++count;
//    }
//  }
