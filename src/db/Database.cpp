...
void Database::dbLock() { this->dbMutex.lock(); }

void Database::dbUnlock() { this->dbMutex.unlock(); }
...
Table &Database::copyTableFromTable(const std::string &srcTableName,
                                    const std::string &dstTableName) {
  auto &db = Database::getInstance();
  auto src = this->tables.find(srcTableName);
  auto &srcTablePtr = src->second;
  if (src == this->tables.end()) {
    throw TableNameNotFound("Error when trying to copy table from \"" +
                            srcTableName + "\". Table not found.");
  }
  auto newTable = std::make_unique<Table>(dstTableName, *srcTablePtr);
  return db.registerTable(std::move(newTable));
}
