...
private:
  ...
  /**
   * The mutex to protect the data in the table
   */
  ...
  // /** reader mutex for divided mutex*/
  // std::vector<ReadWriteLock> mutexes;
...
public:
  ...
  /** when read the table use lock to protect it
   * @param void
   */
  void readLock();

  /** When write the table, lock the table
   * @param void
   */
  void writeLock();
  ...
  /** For partial table query
   * Use reader or writer Mutexes
   */
  void readerLock(int i);
  void writerLock(int i);
   ...
   * @param count
   */
  void deleteByIndex(KeyType key, SizeType count);
   ...
   * @param fieldId2
   */
  void swapByIndex(SizeType count, FieldIndex fieldId1, FieldIndex fieldId2);
   ...
   * @param fieldSum
   */
  void add_subByIndex(SizeType count, FieldIndex fieldDest,
   ...
   * @param counter
   */
  void duplicateByIndex(SizeType count, size_t &counter);
